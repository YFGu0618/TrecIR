...
public class QueryRetrievalModel {
  ...
  public List<Document> retrieveQuery(Classes.Query aQuery, int TopN) throws Exception {
    ...
    Query theQ =
        new QueryParser("CONTENT", new WhitespaceAnalyzer()).parse(aQuery.GetQueryContent());
    ScoreDoc[] scoreDoc = indexSearcher.search(theQ, TopN).scoreDocs;
    for (ScoreDoc score : scoreDoc) {
      results
          .add(new Document(score.doc + "", ireader.document(score.doc).get("DOCNO"), score.score));
    }
    return results;
  }
