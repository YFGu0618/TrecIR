...
public class TrecwebCollection implements DocumentCollection {
	...
	public Map<String, Object> nextDocument() throws IOException {
		...
		if((strLine = brReader.readLine()) != null) {
			...
			while (!strLine.equals("</DOCHDR>")) { // read lines until </DOCHDR>, after which content begins
				strLine = brReader.readLine();
			}
			strLine = brReader.readLine();
			...
			mapDoc.put(strDocID, strDocText.replaceAll(strHTMLTag, " ").replaceAll("( )+", " ").toCharArray());
			return mapDoc;
		}
		if (brReader != null)
			brReader.close();
