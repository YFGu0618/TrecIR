...
public class MyIndexReader {
  ...
  public MyIndexReader(String dataType) throws IOException {
    ...
    // added the collectionStatistics in order to pull
    // the total number of tokens in the collection
    ics = isearcher.collectionStatistics("CONTENT");
  }

  /**
   * Get the (non-negative) integer docid for the requested docno. If -1 returned, it indicates the
   * requested docno does not exist in the index.
   *
   * @param docno
   * @return
   * @throws IOException
  ...
  /**
   * Get the corpus size (number of tokens in the collection)
   * 
   * @return corpus size
   *
   */
